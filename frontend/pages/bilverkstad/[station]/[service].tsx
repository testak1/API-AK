  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-[#090909] text-white selection:bg-red-500">
          ...
          <div className="relative mx-auto max-w-7xl px-5 py-5 sm:px-8 lg:px-10">
            ...
            <div className="grid gap-12 py-20 sm:py-28 lg:grid-cols-[1.1fr_.9fr] lg:items-end">
              <div>
                ...
                <div className="mt-9 flex flex-col gap-3 sm:flex-row"><a href="#boka" className="inline-flex items-center justify-center gap-3 rounded-full bg-red-600 px-7 py-4 text-sm font-extrabold uppercase tracking-wide transition hover:bg-red-500"><CalendarCheck className="h-4 w-4" /> Boka eller fråga</a><a href={`mailto:${stationData.email}`} className="inline-flex items-center justify-center gap-3 rounded-full border border-white/20 px-7 py-4 text-sm font-bold transition hover:bg-white/10"><Mail className="h-4 w-4" /> Mejla verkstaden</a></div>
              </div>
        ...
        <section id="boka" className="scroll-mt-8 mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-10 lg:py-28">
          <div className="rounded-3xl border border-white/10 bg-[#121212] p-8 sm:p-12"><div className="mb-10 grid gap-7 lg:grid-cols-[1fr_auto] lg:items-end"><div><p className="text-xs font-extrabold uppercase tracking-[.2em] text-red-500">Boka en tid</p><h2 className="mt-4 text-4xl font-black uppercase leading-[.94] tracking-[-.05em] sm:text-5xl">Skicka din<br />förfrågan.</h2><p className="mt-6 max-w-xl leading-relaxed text-white/60">Fyll i det du vet så återkommer verkstaden i {stationName} med tid och upplägg. Det tar mindre än en minut.</p></div><a href={bookingHref} className="inline-flex items-center justify-center gap-3 rounded-full border border-white/20 px-7 py-4 text-sm font-extrabold uppercase tracking-wide transition hover:bg-white/10">Ring {stationData.phone} <ArrowRight className="h-4 w-4" /></a></div><WorkshopBookingForm stationSlug={stationData.slug} stationName={stationName} serviceTitle={service.title} /></div>
        </section>
