...
export default async function handler(
  ...
  // --- ÄNDRINGEN ÄR HÄR ---
  // Vi lägger till CORS-proxyn framför den ursprungliga URL:en
  ...
  try {
    ...
    if (vehicleInfoSection.length === 0) {
      console.error(
        "Kunde inte hitta 'Fordonsinformation'-sektionen via proxyn. HTML:",
    ...
    console.error("Fullständigt fel vid scraping via proxy:", error);
    if (axios.isAxiosError(error)) {
      ...
      console.error("Axios felstatus från proxy:", error.response?.status);
    }
    res.status(500).json({
